...
from ....core.realtime import EmployeeRealtimeClient
from ..models import (
    Holiday,
    DropdownOption,
    LeaveDefault,
...
class EmployeeMainWidget(QWidget):
    def __init__(self):
        ...
        # --- Realtime employee updates via backend WebSocket ---
        # This will listen for broadcast events like
        # {"channel": "employees", "action": "created", "data": {...}}
        # and refresh the list when something changes.
        try:
            self._realtime = EmployeeRealtimeClient(self)
            self._realtime.employee_event.connect(self._on_employee_event)
            self._realtime.connection_state_changed.connect(
                self._on_realtime_state_changed
            )
            self._realtime.start()
        except Exception as ex:
            # Fail-safe: if WebSocket setup breaks for any reason,
            # do not crash the entire Employee module.
            print(f"Employee realtime disabled: {ex}")
            self._realtime = None
    ...
    def _reload_employees(self):
        ...
        def _on_employee_event(self, payload: dict) -> None:
            """
            Handle a broadcast event from the backend WebSocket.
            ...
            # For now, on any employees event, just reload the table.
            # Later you can optimise to patch rows instead.
            if action in {"created", "updated", "deleted"}:
                self._reload_employees()

        def _on_realtime_state_changed(self, state: str) -> None:
            """
            Optional hook for debugging WebSocket status.
            Currently just prints to stdout.
            """
            print(f"[employees websocket] {state}")
